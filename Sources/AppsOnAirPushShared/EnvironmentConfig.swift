...
// MARK: - Production Config:
    public static let serverBaseURL = "https://push.appsonair.com/"

// MARK: - API Endpoints:
    public static let registerDevice = serverBaseURL + "v1/subscriptions"

    /// PATCH target for a single subscription. Append the `subscriptionId`:
    ///   subscriptionById + "<subscriptionId>"  →  …/v1/subscriptions/<id>
    public static let subscriptionById = serverBaseURL + "v1/subscriptions/"
    ...
    public static let eventDelivered = serverBaseURL + "v1/events/delivered"

    /// POST target for the open event — sent once the user taps a notification's body.
    public static let eventOpened = serverBaseURL + "v1/events/opened"

    /// POST target for the click event — sent once the user taps a notification's action button.
    public static let eventClicked = serverBaseURL + "v1/events/clicked"
    ...
    public static let startSession = serverBaseURL + "v1/sessions"

    /// PATCH target for ending a session. Append the `sessionId`:
    ///   sessionById + "<sessionId>"  →  …/v1/sessions/<id>
    public static let sessionById = serverBaseURL + "v1/sessions/"
}
